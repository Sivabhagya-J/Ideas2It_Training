...
<body>
    ...
    <div style="margin-bottom:10px">
    	 ...
    	 <a href="listProject" onclick="searchProject(this)">Search</a>
         <a href="listProject" onclick="setStatusParam(this)" style="margin-left:10px">Clear Search</a>
    </div>
    <table border=1>
        <thead>
            <tr>
    			...
    			<c:when test="${projectList != null && projectList.size() > 0}">
        			<c:forEach items="${projectList}" var="project">
		                <tr>
		                    ...
		                    <td><a href="restore?id=<c:out value="${project.id}"/>">Restore</a></td>
		                    </c:if>
		                    <c:if test = "${project.isDeleted == false}">
		                    <td><a href="edit?id=<c:out value="${project.id}"/>">Update</a></td>
		                    <td><a href="delete?id=<c:out value="${project.id}"/>">Delete</a></td>
		                    </c:if>
		                </tr>
            		</c:forEach>
    			</c:when>
    ...
    <p><a href="insert">Add Project</a></p>
